...
public class UI_DeathCard : MonoBehaviour {
	...
	void Update () {
		...
		if((Vector3.Distance(contest.head.position, FindObjectOfType<PlayerController>().pos) < 1.5f || Vector3.Distance(contest.head.position, FindObjectOfType<PlayerController>().transform.position) < 2) && (contest.type == Contestant.ContestantType.AI || contest.type == Contestant.ContestantType.Target)){
